...
//	}
	...
	public void sendPacket(int type, int destId, byte[] data) {
		try {
			synchronized (dos) {
				dos.writeInt(type);
				dos.writeInt(destId);
			...
			while (s != null && !s.isClosed()) {
				...
				notifyMessageListeners(new Packet(type,sender, dest, data));
					...
					if (type == 0) { // Message type
						...
						Path dirPath = Paths.get("/Users/chizhang/Documents/GitHub/chatservice/src/main/resources", String.valueOf(dest));
						...
						// Define the file path, you may want to replace "receivedFile" with actual file name
						Path filePath = dirPath.resolve("receivedFile");
